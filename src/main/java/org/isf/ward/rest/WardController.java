...
public class WardController extends OHApiAbstractController<Ward, WardDTO> {
    ...
    public ResponseEntity<List<WardDTO>> getWards() throws OHServiceException {
        ...
        List<WardDTO> listWard = wards.stream().map(ward -> modelMapper.map(ward, WardDTO.class)).collect(Collectors.toList());
        if (listWard.size() == 0) {
    ...
    public ResponseEntity newWard(@RequestBody WardDTO newWard) throws OHServiceException {
        ...
        boolean isCreated = wardManager.newWard(modelMapper.map(newWard, Ward.class));
        if (!isCreated) {
    ...
    public ResponseEntity updateWard(@RequestBody WardDTO updateWard) throws OHServiceException {
        ...
        boolean isUpdated = wardManager.updateWard(modelMapper.map(updateWard, Ward.class));
        if (!isUpdated) {
    ...
    public ResponseEntity deleteWard(@RequestBody WardDTO wardToDelete) throws OHServiceException {
        ...
        boolean isDeleted = wardManager.deleteWard(modelMapper.map(wardToDelete, Ward.class));
        if (!isDeleted) {
